...
function NavBar() {
    ...
    return (
    <nav className="fixed w-full top-0 z-50 py-3 px-6 lg:px-[60px] shadow-sm bg-[#0F172A] text-white">
            <div className="container py-[12px] mx-auto relative text-sm">
                <div className="flex justify-between items-center">
                    <div className="flex items-center flex-shrink-0">
                        <Link to="/">
                            <img src={Logo} alt=''className='h-[20px] w-auto lg:h-[39px] lg:w-auto' />
                        </Link>
                    </div>
                ...
                {toggle && (
                    <div className="fixed right-0 top-[84px] bottom-0 z-20 bg-[#0F172A] text-xl w-full p-12 flex flex-col justify-top items-center gap-6 lg:hidden">
                       <ul> 
                            {navLinks.map((item, index) => (
